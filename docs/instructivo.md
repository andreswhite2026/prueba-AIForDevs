 ...
 5) Migración a base de datos (opcional)

 Si quieres usar la base relacional, ejecuta `db/schema.sql` en PostgreSQL:

 ```bash
 psql -d tu_basedatos -f db/schema.sql
 ```

 6) Capturas de pantalla para la entrega

 - Tomar capturas de la `Pantalla principal` y del formulario de inscripción y guardarlas en `docs/mockups/`.
 - Si quieres que las incluya en el instructivo, súbelas a `docs/mockups/` y actualizaré el instructivo para referenciarlas.
 ...
 Soporte
 -------
 Si quieres, creo un PDF del instructivo o subo capturas si me las proporcionas.
